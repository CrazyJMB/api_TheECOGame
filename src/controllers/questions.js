...
const getRandomQuestionForSpecificUser = async (req, res) => {
  try {
    ...
    const [rows] = await db.pool.query(
      "SELECT q.*, (SELECT qo.ods_id FROM question_ods qo WHERE qo.question_id = q.challenge_details_id LIMIT 1) AS ods_id FROM question q where difficulty = ? and challenge_details_id not in (Select challenge_id from game_challenge where challenge_type like 'QUESTION' and game_id in (Select user_id from game where user_id = ?)) ORDER by Rand() limit 1",
      [dif, userId]
    );
    if (rows.length <= 0) {
      return handleErrorResponse(res, "Question not found", 404);
